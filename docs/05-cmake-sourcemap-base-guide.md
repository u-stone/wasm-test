...
## 15. 本地 HTTP 根目录与磁盘路径如何对应
很多人第一次做 wasm sourcemap 调试时，会把“磁盘路径”和“浏览器 URL”混为一谈。实际情况是：

1. 浏览器只认识 URL，不认识你的 `D:\`、`E:\` 盘符。
2. 本地磁盘路径必须先通过 HTTP 服务映射成 URL 路径，DevTools 才能使用。
3. `--source-map-base` 负责告诉浏览器“应该按哪个 URL 根去理解这些调试资源”。

这意味着真正需要对齐的不是“磁盘路径对磁盘路径”，而是：

1. 磁盘上的产物目录
2. HTTP 服务暴露出来的 URL 根
3. `--source-map-base` 生成的 URL 前缀

### 15.1 本仓库里的对应关系

本仓库默认从仓库根目录启动本地服务，因此：

1. HTTP 服务根目录是仓库根目录。
2. URL 路径等于“文件相对于仓库根目录的相对路径”。
...
```text
D:\MyGitHub\wasm\demos\05-cmake-emcmake\output\sourcemap\cmake_demo.wasm.map
```

2. 对应 URL：

```text
http://localhost:8000/demos/05-cmake-emcmake/output/sourcemap/cmake_demo.wasm.map
```

这就是为什么“本地 HTTP 服务从哪里启动”是有讲究的：

1. 如果你从仓库根启动，URL 根就对应仓库根。
2. 如果你从更深一层子目录启动，URL 根会变化。
3. 一旦 URL 根变化，而 `--source-map-base` 仍按旧规则生成，浏览器就会去错误位置找 map 和源码。

### 15.2 本地开发时的推荐做法

推荐固定一个稳定的 HTTP 根目录，然后让构建产物和源码相对这个根目录保持稳定映射。

最简单的规则可以概括为：

1. HTTP 服务根目录 = 项目根目录或统一的 debug staging 目录。
2. URL 路径 = 文件相对于这个根目录的相对路径。
3. `--source-map-base` = 这个 HTTP 根目录对应的 URL 前缀。
...
2. 明天从某个子目录启动服务。
3. 后天又换一个临时目录启动。
4. 但 sourcemap URL 规则不变。

这种情况下，浏览器有时能调试、有时不能，本质上就是 URL 根在漂移。

## 16. Windows 多磁盘 / 多仓库场景下的推荐做法
如果大型工程里静态库源码、编译目录、最终 Wasm 产物分散在多个磁盘，例如：

1. `D:\libA\...`
2. `E:\libB\...`
3. `F:\final-app\...`

不要指望浏览器直接理解这些机器本地绝对路径。业界更推荐把问题拆成两层治理：

### 16.1 构建输入可以分散，调试资源输出必须收口

推荐做法是：

1. 构建输入允许来自不同仓库、不同磁盘。
2. 但最终 `.wasm`、`.wasm.map`、以及需要暴露给 sourcemap 的源码镜像或源码包，统一收口到一个 debug artifact staging 目录。
3. 浏览器最终只面向这个统一目录对应的 URL 根，而不是面向构建机的 `D:\` / `E:\` / `F:\`。

可以把这件事理解为：

```text
Build disks -> debug artifact staging -> stable HTTP URL root
```

而不是：

```text
Build disks -> browser directly reads local disk paths
```

### 16.2 为什么不推荐直接暴露机器绝对路径

`C:\...`、`D:\...`、`E:\...` 这种路径不适合作为长期调试路径，原因包括：

1. 它们是机器相关的，一换机器就失效。
2. 一换 CI 机器或构建目录，路径就会漂移。
3. 这类路径不可发布、不可复现、不可协作。

因此更推荐：

1. 编译阶段保留调试信息。
2. 发布阶段统一调试资源目录。
3. 通过 `--source-map-base` 把 URL 根固定下来。
4. 让浏览器始终通过 URL 找资源，而不是隐式依赖构建机本地盘符。

### 16.3 业界常见工程实践

大型工程里更常见的做法通常是以下几种之一：

1. 把需要调试的源码副本随 `.wasm.map` 一起发布到统一的 debug asset 根目录。
2. 在 CI 中把不同仓库、不同磁盘上的源码收拢到统一 staging 目录，再生成和发布 map。
3. 对构建机本地路径做前缀重写或逻辑路径规范化，让调试资源最终落到统一 URL 空间下。

一个更工程化的目标应该是：

1. 编译输入可以分散。
2. 调试资源输出必须统一。
3. 浏览器只依赖统一的 URL 根，不依赖构建机本地盘符。

### 16.4 实际排障时该怎么判断

如果你在 Windows 多磁盘场景里发现浏览器调不到源码，可以按下面顺序判断：

1. 最终 `.wasm.map` 是否已经统一收口到单一可访问目录。
2. HTTP 服务是否就是从这个统一目录或它的上层稳定根目录启动的。
3. `--source-map-base` 是否指向这个统一目录对应的 URL 前缀。
4. `.wasm.map` 中的源码条目是否最终都能映射到可访问的 HTTP 资源，而不是只能在某台机器的某个盘符下找到。

如果这四步中有任意一步不成立，DevTools 的源码映射通常就会不稳定，甚至直接失效。